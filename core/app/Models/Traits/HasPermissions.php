...
/** Super admin short-circuit */
    ...
    //     $abilities = array_map('strtolower', $abilities);
    //     $map = ['view'=>'can_view','add'=>'can_add','edit'=>'can_edit','delete'=>'can_delete','export'=>'can_export'];

    //     $keyById = $this->permKeyMap(); // [id => 'usermanage.users']

    //     // 1) user overrides first (any allow wins)
    //     foreach ($this->userPermCache() as $pid => $row) {
    //         $key = $keyById[$pid] ?? null;
    //         if (!$key || !Str::startsWith($key, $prefix)) continue;
    //         foreach ($abilities as $ab) {
    //             $col = $map[$ab] ?? 'can_view';
    //             if ($row->{$col} !== null && (bool)$row->{$col} === true) return true;
    //         }
    //     }
    //     // 2) role baseline
    //     foreach ($this->rolePermCache() as $pid => $row) {
    //         $key = $keyById[$pid] ?? null;
    //         if (!$key || !Str::startsWith($key, $prefix)) continue;
    //         foreach ($abilities as $ab) {
    //             $col = $map[$ab] ?? 'can_view';
    //             if ((bool)($row->{$col} ?? false)) return true;
    //         }
    //     }
    //     return false;
    // }
